...
export async function saveSession(sessionData: DetectionSessionData): Promise<boolean> {
  ...
  try {
    // 验证必要字段
    if (!sessionData.id || !sessionData.websites || !Array.isArray(sessionData.websites)) {
      console.error("保存会话数据失败: 数据格式无效", sessionData)
      return false
    }
    ...
    if (existingIndex >= 0) {
      // 更新现有会话，但保留原始创建时间
      const originalCreatedAt = sessions[existingIndex].createdAt
      sessions[existingIndex] = {
        ...sessionData,
        createdAt: originalCreatedAt || sessionData.createdAt,
      }
    } else {
      // 添加新会话，确保有创建时间
